    ...
    && $_POST['username'] && $_POST['password'] && $_POST['againpass'] && $_POST['firstname'] && $_POST['lastname'])
{
   ...
   else if ($new_id = Users::create_user($_POST['username'], $_POST['password'], $_POST['firstname'], $_POST['lastname'], False))
   {
	  ...
	  $if_success = $_GET['if_success'];
	  	if($if_success=="")
	  		 {
	    		echo("Not a valid URL!!");
	    		echo("<br>Redirecting to user registration again.. .");
	    		sleep(3);
	    		header( "refresh:3;url=register.php?if_success=http://$url1/users/home.php" );
	  		 }
	  	else 
	  		 {
	    echo("Goto: ".$if_success);
	    header('Location: '.$if_success);
	  		 }
	  sleep(2);
   }
   else
